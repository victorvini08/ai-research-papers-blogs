        ...
        for attempt in range(max_retries):
            try:
                ...
                if os.path.getsize(tmp_file.name) > min_size_bytes:
                    ...
                    raise Exception(f"Downloaded file is too small ({os.path.getsize(tmp_file.name)} bytes), likely corrupted")
    ...
    def _extract_contributions_robust(self, text: str) -> str:
        ...
        if intro_text:
            ...
            for sentence in sentences:
                ...
                if any(keyword in sentence_lower for keyword in ['contribution', 'introduce', 'propose', 'develop', 'novel']):
                    if len(sentence) > 20:
